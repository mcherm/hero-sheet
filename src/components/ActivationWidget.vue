...
-->
<template>
  <div :class="{'activate-widget': true, 'horizontal': horizontal}">
    <svg
        xmlns="http://www.w3.org/2000/svg"
        :width="horizontal ? 32 : 16"
        :height="horizontal ? 16 : 32"
        :viewBox="horizontal ? '0 0 1024 512' : '0 0 512 1024'"
    >
      <g :transform="horizontal ? 'rotate(-90) translate(-512,0)' : ''">
    ...
    <div class="active-ranks-setting" v-if="activation.activationStatus === 'partial'">
      <edit-button class="active-ranks-button incr" :onClick="() => clickIncr()">+</edit-button>
      <number-display class="active-ranks" :value="activation.ranks" :used-in-play="true"/>
      <edit-button class="active-ranks-button decr" :onClick="() => clickDecr()">-</edit-button>
    </div>
  </div>
</template>
